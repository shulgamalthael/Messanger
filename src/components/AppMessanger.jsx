import React, { useEffect, useState } from 'react';
import { Route, Switch, useParams } from 'react-router-dom';
import Messanger from './Messanger/Messanger.jsx';
import { connect } from 'react-redux';
import * as clientAcitions from '../redux-store/actions/client.actions.js';
import { clientDataSelector, logginedUserSelector } from '../redux-store/selectors/client.selectors.js'; 

export const defaultAvatar_url = "data:image/png;base64,";

const AppMessanger = ({ ws, getClientData, clientData, loginnedUser }) => {
    const [ user, setUser ] = useState(null);

    useEffect(() => {
        setUser(loginnedUser)
    }, [])

    const { id } = useParams();

    const CompanionData = {
        id: 'Example',
        avatar: defaultAvatar_url,
    };

    useEffect( () => getClientData(), [])

    return(
        <Switch>
            <Route path={`/${user}/conversations/${id}`}>
                <Messanger
                    ws={ws}
                    currentCompId={id}
                    loginnedUser={loginnedUser}
                    CompanionData={CompanionData} 
                    clientData={clientData}
                />
            </Route>
        </Switch>
    )
}

const mapState = state => {
    return {
        clientData: clientDataSelector(state),
        loginnedUser: logginedUserSelector(state),
    }
}

const mapDispatch = {
    getClientData: () => clientAcitions.get_client_data(),
}


export default connect( mapState, mapDispatch )( AppMessanger );